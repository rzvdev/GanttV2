    ...
    public partial class Produzione : Form
    {
        ...
        private void CaricoLav_Load(object sender, EventArgs e)
        {
            ...
            dgvReport.DataBindingComplete += (s, events) =>
            {
                ...
                dgvReport.Rows[0].DefaultCellStyle.BackColor = Color.WhiteSmoke;
                dgvReport.Rows[0].DefaultCellStyle.SelectionBackColor = Color.WhiteSmoke;
                dgvReport.Rows[0].DefaultCellStyle.SelectionForeColor = Color.Green;
                dgvReport.Rows[0].Cells[0].Style.ForeColor = Color.Black;
                dgvReport.Rows[0].Cells[0].Style.SelectionBackColor = Color.WhiteSmoke;
                dgvReport.Rows[0].Cells[0].Style.SelectionForeColor = Color.Black;
                dgvReport.Rows[0].Height = 20;
                dgvReport.Rows[0].DefaultCellStyle.Alignment = DataGridViewContentAlignment.BottomLeft;
                dgvReport.Rows[0].Frozen = true;

                dgvReport.Rows[1].DefaultCellStyle.Font = new Font("Microsoft Sans Serif", 8, FontStyle.Bold);
                dgvReport.Rows[1].DefaultCellStyle.ForeColor = Color.Red;
                dgvReport.Rows[1].DefaultCellStyle.BackColor = Color.WhiteSmoke;
                dgvReport.Rows[1].DefaultCellStyle.SelectionBackColor = Color.WhiteSmoke;
                dgvReport.Rows[1].DefaultCellStyle.SelectionForeColor = Color.Red;
                dgvReport.Rows[1].Cells[0].Style.ForeColor = Color.Black;
                dgvReport.Rows[1].Cells[0].Style.SelectionBackColor = Color.WhiteSmoke;
                dgvReport.Rows[1].Cells[0].Style.SelectionForeColor = Color.Black;
                dgvReport.Rows[1].Height = 20;
                dgvReport.Rows[1].DefaultCellStyle.Alignment = DataGridViewContentAlignment.BottomLeft;
                dgvReport.Rows[1].Frozen = true;

                dgvReport.Rows[2].DefaultCellStyle.Font = new Font("Microsoft Sans Serif", 8, FontStyle.Bold);
                dgvReport.Rows[2].DefaultCellStyle.ForeColor = Color.SteelBlue;
                dgvReport.Rows[2].DefaultCellStyle.BackColor = Color.WhiteSmoke;
                dgvReport.Rows[2].DefaultCellStyle.SelectionBackColor = Color.WhiteSmoke;
                dgvReport.Rows[2].DefaultCellStyle.SelectionForeColor = Color.SteelBlue;
                dgvReport.Rows[2].Cells[0].Style.ForeColor = Color.Black;
                dgvReport.Rows[2].Cells[0].Style.SelectionBackColor = Color.WhiteSmoke;
                dgvReport.Rows[2].Cells[0].Style.SelectionForeColor = Color.Black;
                dgvReport.Rows[2].Height = 20;
                dgvReport.Rows[2].DefaultCellStyle.Alignment = DataGridViewContentAlignment.BottomLeft;
                ...
                for (var i = 0; i <= dgvReport.Columns.Count - 1; i++)
                {
                    ...
                    dgvReport.Columns[i].HeaderCell.Style.BackColor = Color.Gold;
                    dgvReport.Columns[i].HeaderCell.Style.Font = new Font("Microsoft Sans Serif", 8, FontStyle.Bold);
                    dgvReport.Columns[i].HeaderCell.Style.Alignment = DataGridViewContentAlignment.BottomLeft;
                    dgvReport.Columns[i].DefaultCellStyle.Font = new Font("Microsoft Sans Serif", 8);
                ...
                for (var i = 0; i <= 1; i++)
                {
                    ...
                    dgvReport.Columns[i].DefaultCellStyle.BackColor = Color.Gold;
                    dgvReport.Columns[i].HeaderCell.Style.Alignment = DataGridViewContentAlignment.BottomLeft;
                    dgvReport.Columns[i].HeaderCell.Style.BackColor = Color.Gold;
                    dgvReport.Columns[i].HeaderCell.Style.Font = new Font("Microsoft Sans Serif", 8, FontStyle.Bold);
        ...
        public void LoadReportTable()
        {
            ...
            foreach (DataRow line in lines)
                for (var c = 1; c <= _columnRange; c++)
                {
                    ...
                    if (oldLine != newLine)
                    {
                        if (sepCount == 3)
                        {
                            table_report.Columns.Add("sep_" + newLine + index.ToString());
                            sepCount = 0;
                        }
            ...
            var dateBefore = new DateTime();
            var firstRead = true;
            var lineQty = 0;
            foreach (DataRow row in table_data.Rows)
            {                
                var date = Convert.ToDateTime(row[0]);
                var comm = row[1].ToString();
                var capi = row[2].ToString();
                ...
                if (date != dateBefore)
                {
                    //lineQty = 0;
                    partIndex = 0;
                    repRow = table_report.NewRow();
                    repRow[0] = date.ToString("dd/MM");

                    if (!firstRead && date.DayOfWeek == DayOfWeek.Monday)
                    {
                    ...
                    if (lineBefore == line)
                    {
                        ...
                        lineQty += q;
                        partIndex++;
                        repRow[strComm + "_" + line + "_" + partIndex.ToString()] = comm;
                        repRow[strCapi + "_" + line + "_" + partIndex.ToString()] = capi.ToString();
                        //lineQty = 0;
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        partIndex = 1;
                        repRow[strComm + "_" + line + "_" + partIndex.ToString()] = comm;
                        repRow[strCapi + "_" + line + "_" + partIndex.ToString()] = capi;
                        table_report.Rows[0][strCapi + "_" + line + "_" + "1"] = lineQty.ToString();
                    }
                }
            ...
            for (var c = 2; c <= dgvReport.Columns.Count - 1; c++)
            {
                ...
                for (var r = 1; r <= dgvReport.Rows.Count - 1; r++)
                {
                    int.TryParse(dgvReport.Rows[r].Cells[c].Value.ToString(), out var tot);
                    total += tot;
                    if (tot > 0) x++;                   
                }
                if (x == 0) x = 1;
                dgvReport.Rows[2].Cells[c].Value =
                    string.Format("{0:#,##0}", (total / x).ToString());
            }
            rowHeight = 0;
        ...
        private void dgvReport_Paint(object sender, PaintEventArgs e)
        {
            ...
            for (int j = 0; j < dgvReport.ColumnCount - 1;)
            {
                ...
                _headerFont = new Font("Microsoft Sans Serif", 8, FontStyle.Bold);

                if (txt == "ANALISI CONSEGNE")
                {
                    _headerFont = new Font("Microsoft Sans Serif", 10, FontStyle.Bold);
                    brsh = Brushes.Blue;
                    e.Graphics.FillRectangle(specBrush, _rect);
                }
                else
                {
                    ...
                    var sector = Store.Default.selDept.Split(' ')[1];
                ...
                if (txt == "ANALISI CONSEGNE")
                {
                    //e.Graphics.DrawLine(Pens.Gray, _rect.X, _rect.Y - 1, _rect.Width, _rect.Y - 1);
                    j += 2;
                }
...
//    public string Capi { get; set; }

//    public string Line { get; set; }

//    public string Partition { get; set; }

//    public Partitions(string date, string comm, string capi, string line, string partition)
//        {
//        Date = date;
//        Comm = comm;
//        Capi = capi;
