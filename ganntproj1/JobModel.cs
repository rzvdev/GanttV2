namespace ganntproj1
{
    ...
    public class JobModel
    {
        ...
        /// <summary>
        /// The SkipWeekendRange
        /// </summary>
        /// <param name="dateFrom"></param>
        /// <param name="dateTo"></param>
        /// <returns></returns>
        public static int SkipWeekendRange(DateTime dateFrom,
                                    DateTime dateTo)
        {
            if (dateTo.Date.Subtract(dateFrom.Date).Days == 9) return 0;
            var i = 0;
        ...
        /// <returns></returns>
        public double CalculateDailyQty(string aim, double qtyH)
        {
            var linesQuery = from lines in ObjectModels.Tables.Lines
                             where lines.Line == aim && lines.Department == Store.Default.selDept
                             select lines;
            ...
            return Math.Round(lineMembers * qtyH * 7.5 * abatimento, 0);
        }
                                     ...
                                     double qtyH)
        {
            ...
            var duration = Math.Round
                (Convert.ToDouble(qty / (lineMembers * qtyH * 7.5 * abatimento)), 0);
            if (duration <= 0)
            {
                duration = 1;
            }
            ...
            string aim)
        {
            ...
            var production = from prod in ObjectModels.Tables.Productions
            ...
            using (var context = new System.Data.Linq.DataContext(Central.SpecialConnStr))
            {
                ...
                context.ExecuteCommand("update objects set " +
                    ...
                    "where ordername={6} and aim={7}", 
                    JobModel.GetLSpan(prodStart),
                    JobModel.GetLSpan(endDate), 
                    dymQty,dymAlertTime.Ticks,JobModel.GetLSpan(endDate),JobModel.GetLSpan(endDate) + dymAlertTime.Ticks, 
                    job,aim);
            }
        ...
        /// <returns></returns>
        public static DateTime GetLineLastDate(string line)
        {
            var query = (from mods in Central.ListOfModels
                         where mods.Aim == line
                         orderby mods.EndDate
                         select mods).ToList();
            var date = DateTime.MinValue;
            if (query.Count > 0)
                date = query.Select(x => x.EndDate).Max();
        ...
        {
            var artQuery = from art in ObjectModels.Tables.Articles
    ...
    public class Index
    {
        ...
        public Index(int rowIndex, int objIndex, string objText, string objAim)
        {
            RowIndex = rowIndex;
            ObjIndex = objIndex;
            ObjText = objText;
    ...
    public class LineHolidaysEmbeded
    {
        ...
        /// <param name="m"></param>
        /// <param name="y"></param>
        public LineHolidaysEmbeded(string line, string hld, int m, int y)
        {
            Line = line;
            HolidayArray = hld;
            Month = m;
    ...
    public class ArticlesUnit
    {
        ...
        public ArticlesUnit(string art, double qh, double price)
        {
            Article = art;
            QtyH = qh;
            Price = price;
    ...
    public class LinesUnit
    {
        public LinesUnit() { }
        public LinesUnit(string art, int members, double abatimento)
        {
            Line = art;
            Memebers = members;
