namespace ganntproj1
{
    ...
    internal class Ganttogram : UserControl
    {
        ...
        /// </summary>
        public object MouseOverRowValue { get; private set; }
        ...
        /// </summary>
        private Color ChartBackColor { get; } = Color.FromArgb(245, 245, 245);//Color.FromArgb(250, 250, 250);

        /// <summary>
        /// Gets the ChartBackBrush
        /// </summary>
        private Brush ChartBackBrush { get; } = new SolidBrush(Color.FromArgb(245, 245, 245));

        /// <summary>
        /// Defines the _barFont
        /// </summary>
        private readonly Font _barFont = new Font("Microsoft Sans Serif", 7, FontStyle.Bold);
        ...
        private void DrawHeader(Graphics gfx, List<Header> headerList)
        {
            while (true)
            {
                ...
                gfx.FillRectangle(new SolidBrush(Color.FromArgb(235, 235, 235)), new Rectangle(0, 0, Width - 40, HeaderHeight));
                ...
                foreach (var header in headerList)
                {
                    ...
                    var month = header.HeaderTextInsteadOfTime.Length > 0
                            ? header.HeaderTextInsteadOfTime
                            : header.Time.ToString("MMM");

                    //measure header text
                    var mesHText = gfx.MeasureString(day, DateFont);
                    var hgHText = mesHText.Height;

                    dynamic fontSize = FromDate.Day == dayMax ? 10 : 12;
                    var lineWdth = 2;
                    if (fontSize == 10) lineWdth = 1;
                    ...
                    while (curMonth != month)
                    {
                        ...
                        gfx.DrawString(month, new Font("Tahoma", fontSize, FontStyle.Bold), Brushes.Gray, startPos + 5, HeaderHeight - hgHText - 36);
        ...
        public void AddBars(string rowText, string chain, object barValue,
            ...
            DateTime prodOverStartValue, DateTime prodOverEndValue, bool locked, bool prodLocked, bool closed, Color prodColor, string art)
        {
        ...
        private void DrawBars(Graphics grfx, bool ignoreScrollAndMousePosition = false)
        {
            ...
            foreach (var bar in Bars)
            {
                ...
                //if (!RectangleSelectorActivated)
                //    {

                //    }
                //else
                //    {
                //    var rxc = MouseOverColumnDate.Subtract(bar.StartValue).TotalDays;
                //    var rx = _barStartLeft + GetBarStartLocation(bar.StartValue.AddDays(+rxc));

                //    var p = new Pen(Brushes.Gray, 2);
                //    p.DashStyle = DashStyle.Dot;
                //    grfx.FillRectangle(new SolidBrush(Color.FromArgb(70, 15, 35, 240)),
                //        rx - (_widthPerItem / 2), y - 5, _widthPerItem - 1, h + 15);

                //    grfx.DrawRectangle(p,
                //        rx - (_widthPerItem / 2), y - 5, _widthPerItem - 1, h + 15);
                //    grfx.DrawString(bar.RowText + "\n" + MouseOverColumnDate.ToShortDateString() + "\n" + MouseOverColumnDate.ToString("HH:mm"),
                //        new Font("Microsoft Sans Serif", 7,
                //        FontStyle.Regular),
                //        Brushes.Black, rx + _widthPerItem - 5, y + 25);
                //    }                   
                         ...
                         ignoreScrollAndMousePosition)
                {
                    ...
                    LinearGradientBrush linGrBrush = new LinearGradientBrush(
                       barRect,
                        ...
                        Color.FromArgb(255, 80, 80, 80), 90.0f, true);  // Opaque red

                    float[] relativeIntensities = { 0.0f, 0.7f, 1.0f };
                    ...
                    if (HideClosedTask && barColor != Color.FromArgb(27, 98, 124) && barColor != Color.DodgerBlue) continue;

                    grfx.FillPath(linGrBrush, geo.RoundedRectanglePath(barRect, 3));
                    grfx.DrawPath(borderPen, geo.RoundedRectanglePath(barRect, 3));
                    ...
                    if (!HideDelay)
                    {
                        if (bar.DelayStartValue != DateTime.MinValue && bar.DelayEndValue != DateTime.MinValue && bar.DelayStartValue < bar.DelayEndValue)
                        {
                            ...
                            barRect,
                            Color.Orange,     // Opaque black                              
                            Color.DarkOrange, 90.0f, true);  // Opaque red

                            relativeIntensities = new[] { 0.0f, 0.7f, 1.0f };
                            relativePositions = new[] { 0.0f, 0.7f, 1.0f };
                            ...
                            blend.Positions = relativePositions;
                            linGrBrush.Blend = blend;
                            grfx.FillPath(linGrBrush, geo.RoundedRectanglePath(delayBarRect, 3));
                            grfx.DrawPath(borderPen, geo.RoundedRectanglePath(delayBarRect, 3));
                        }
                    }
                    grfx.FillPath(new SolidBrush(bar.ProdColor), geo.RoundedRectanglePath(prodBarRect, 3));
                    grfx.DrawPath(borderPen, geo.RoundedRectanglePath(prodBarRect, 3));
                    ...
                    if (_barHeight >= 36)
                    {
                        ...
                        if (w > 100)
                        {
                            ...
                            if (bar.Locked && bar.Color != Color.DarkOrange)
                    ...
                    grfx.FillRectangle(new SolidBrush(Color.FromArgb(235, 235, 235)), lineRect);

                    // Draws sub-task image
                    using (Image subTask = Properties.Resources.main_task ??
                                           throw new ArgumentNullException(nameof(grfx)))
                    {
                        var destRect1 = new Rectangle(13,
                            (BarStartTop + _barHeight * (index - scrollPos) + _barSpace * (index - scrollPos) + 1) + _barHeight / 2 - 5, 16, 16);
                        const GraphicsUnit units = GraphicsUnit.Pixel;

                        grfx.DrawImage(subTask, destRect1, 0, 0, 16, 16, units);
                    }

                    // Draws strings as row texts/values
                    grfx.DrawString(new string(' ', 8) + bar.Tag, new Font("Microsoft Sans Serif", 10, FontStyle.Regular),
                        Brushes.Black, 3,
                        (BarStartTop + _barHeight * (index - scrollPos) + _barSpace * (index - scrollPos) + 1) + _barHeight / 2 - 4);
                }

                // Draws the scroll area
                var maxHeight = Height - 30;
                _scrollBarArea = new Rectangle(Width - 40, 0, 45, maxHeight + 10);
                grfx.FillRectangle(new SolidBrush(Color.FromArgb(235, 235, 235)), _scrollBarArea);
            }
        }
            ...
            foreach (var header in HeaderList)
            {
                ...
                foreach (var item in Central.ListOfHolidays)
                {
                    ...
                    foreach (var bar in Bars)
                    {
                        ...
                        {
                            if (item.Line != "LINEA1") continue;

                            grfx.FillRectangle(new SolidBrush(Color.FromArgb(50, 249, 139, 49)),
                          _barStartLeft + index * _widthPerItem + 1,  //x
        ...
        public void Chart_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            foreach (var bar in Bars)
            {
                ...
                //if ((LocalMousePosition.Y > bar.DelayTopLocation.Left.Y) &
                // (LocalMousePosition.Y < bar.DelayBottomLocation.Left.Y))
                //{
                //    if ((LocalMousePosition.X > bar.DelayTopLocation.Left.X) &
                //        (LocalMousePosition.X < bar.DelayTopLocation.Right.X))
                //    {
                //        // If the current bar is the one where the mouse is above, the rowText and rowValue needs to be set correctly
    ...
    public class Bar
    {
        ...
        public Bar(string rowText, string chain, DateTime fromTime, DateTime toTime, DateTime prodFromTime, DateTime prodToTime, DateTime delayStart, DateTime delayEnd,
            ...
            int fixQty, int dailyQty, int prodQty, DateTime prodOverFromTime, DateTime prodOverToTime, bool locked, bool prodLocked, bool closed, Color prodColor, string art)
        {
            RowText = rowText;
            Chain = chain;
            FromTime = fromTime;
