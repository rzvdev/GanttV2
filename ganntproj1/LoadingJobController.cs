namespace ganntproj1
{
    ...
    /// <summary>
    /// Defines the <see cref="LoadingJobController" />
    /// </summary>
    public partial class LoadingJobController : Form
    {
        /// <summary>
        /// Defines the _config
        /// </summary>
        ...
        /// <summary>
        /// Defines the _listOfAllLines
        /// </summary>
        private List<string> _listOfAllLines = new List<string>();
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="LoadingJobController"/> class.
        /// </summary>
        public LoadingJobController()
        {
            InitializeComponent();
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            _connection = new SqlConnection(Central.ConnStr);
            _lstOfListedArts = new List<string>();
            HasOrderSelected = false;
            LoadCaricoLavoroInternal();
            // Stores all registered lines to list
            _listOfAllLines = new List<string>();
            var linesQuery = from lines in ObjectModels.Tables.Lines
                             select lines;
            foreach (var line in linesQuery.ToList())
            {
                _listOfAllLines.Add(line.Line);
            }
            if (Central.IsProgramare)
            {
                ...
                foreach (var actArt in WorkflowController.LineActiveArticlesList)
                {
                    foreach (DataRow row in _tableCarico.Rows)
                    {
                        if (row[2].ToString() != actArt) continue;
                        ...
                        if (qtyH < 1.0 || qty <= 0) continue;
                        var lbl = new Label
                        {
                            Name = row[1].ToString() + "_" + row[2].ToString(),
                            BackColor = Color.FromArgb(27, 98, 124),
                        ...
                        var dur = jMod.CalculateJobDuration(WorkflowController.TargetLine, qty, qtyH);
                        ...
                        lbl.MouseLeave += (s, mv) =>
                        {
                            lbl.BackColor = Color.FromArgb(27, 98, 124);
                            foreach (DataGridViewRow dRow in dgvReport.Rows)
                            {
                                dRow.DefaultCellStyle.BackColor = Color.FromArgb(235, 235, 235);
                            }
                        ...
                        lbl.Click += (s, cv) =>
                        {
                            ...
                            WorkflowController.ByQty = byQty;
                            WorkflowController.TargetOrder = lbl.Name.Split('_')[0];
                            Close();
                        };
            ...
            dgvReport.DataBindingComplete += (s, ev) =>
            {
                ...
                dgvReport.Columns["Ritardo"].DisplayIndex = 14;
                dgvReport.Columns["Ritardo"].HeaderCell.Style.BackColor = Color.LightBlue;
        ...
        {
            WorkflowController.ListOfRemovedOrders.Clear();
            LoadCaricoLavoroInternal();
        }
        ...
        private void LoadCaricoLavoroInternal()
        {
            ...
            var sector = Store.Default.selDept;
            ...
            {
                queryCondition = "where Comenzi.DataProduzione is null and Comenzi.department= '" + sector + "' " +
                    "and Comenzi.IdStare<>2 or Comenzi.Line is null and Comenzi.department= '" + sector + "' " +
                    "and Comenzi.IdStare<>2 or Comenzi.IdStare=4 and Comenzi.department= '" + sector + "' " +
                    "and Comenzi.IdStare<>2 and Comenzi.DataProduzione is null " +
                    "and Comenzi.Line is null " +
                    "order by case when Comenzi.Rdd is null then 1 else 0 end, Comenzi.Rdd";
            }
            if (UseSingleFilter)
                queryCondition = "where Comenzi.NrComanda='" + WorkflowController.TargetOrder
                    + "' and Comenzi.IdSector='" + 1.ToString() + "'";
            ...
            foreach (DataRow row in table.Rows)
            {
                ...
                if (arrivo != DateTime.MinValue && start != DateTime.MinValue)
                {
                    var duration = start.Subtract(arrivo).Days;
            ...
            dgvReport.Columns[0].HeaderCell.Style.BackColor = Color.FromArgb(235, 235, 235);
            dgvReport.Columns["FlagA"].Width = 35;
            dgvReport.Columns["FlagA"].HeaderText = "Info";
            dgvReport.Columns["FlagA"].HeaderCell.Style.BackColor = Color.FromArgb(235, 235, 235);
            dgvReport.Columns["FlagA"].DisplayIndex = 1;
            dgvReport.Columns["FlagB"].Width = 35;
            dgvReport.Columns["FlagB"].HeaderText = "Time";
            dgvReport.Columns["FlagB"].HeaderCell.Style.BackColor = Color.FromArgb(235, 235, 235);
            ...
            dgvReport.Columns["TempStat"].HeaderCell.Style.BackColor = Color.LightBlue;
            dgvReport.Columns["TempStat"].Width = 60;

            dgvReport.Columns[1].HeaderText = "Commessa";
            dgvReport.Columns[1].Frozen = true;
            dgvReport.Columns[1].HeaderCell.Style.BackColor = Color.SkyBlue;
            dgvReport.Columns[2].HeaderText = "Aricoli";
            dgvReport.Columns[2].Frozen = true;
            dgvReport.Columns[2].HeaderCell.Style.BackColor = Color.SkyBlue;
            dgvReport.Columns[3].HeaderText = "Linea";
            dgvReport.Columns[3].Frozen = true;
            dgvReport.Columns[3].HeaderCell.Style.BackColor = Color.SkyBlue;
            ...
            dgvReport.Columns["Ritardo"].DisplayIndex = 16;
            dgvReport.Columns["Ritardo"].HeaderCell.Style.BackColor = Color.LightBlue;
            dgvReport.Columns["Ritardo"].Width = 60;
            dgvReport.Columns["CaricoTrigger"].Visible = false;
            dgvReport.Columns[26].DisplayIndex = 11;
            dgvReport.Columns[27].DisplayIndex = 13;

            if (Store.Default.selDept == "Confezione")
            {
            ...
            //for (var x = 4; x <= 16; x++)
            //{
            //    if (x == 4 || x == 5 || x == 6) continue;
            //    dgvReport.Columns[x].Width = 40;
            //}
            for (var x = 7; x <= 12; x++)
            {
            ...
            for (var i = 4; i <= 12; i++)
                dgvReport.Columns[i].HeaderCell.Style.BackColor = Color.LightBlue;

            dgvReport.Columns[26].HeaderCell.Style.BackColor = Color.LightBlue;
            dgvReport.Columns[27].HeaderCell.Style.BackColor = Color.LightBlue;
            dgvReport.Columns[26].DefaultCellStyle.BackColor = Color.WhiteSmoke;
            dgvReport.Columns[27].DefaultCellStyle.BackColor = Color.WhiteSmoke;
        ...
        {
            var query = from split in ObjectModels.Tables.ProductionSplits
                        select split;
            var lstOfSplit = query.ToList();
            var lst = new List<string>();
            foreach (var str in lstOfSplit)
            {
                if (!lst.Contains(str.Commessa.Trim())) lst.Add(str.Commessa.Trim());
            }
            return lst;
        }
        ...
        private Image ReturnImageByState(string ord)
        {
            ...
            Image img3 = Properties.Resources.order_statusok_24;
            Bitmap bmp = new Bitmap(24, 24); //empty
            Image img = bmp;
            if (_lstOfSplittedOrd.Contains(ord)) img = img1;
            else if (WorkflowController.ListOfRemovedOrders.Contains(ord)) img = img2;
        ...
        public void PrintGrid()
        {
            ...
            dgvReport.Columns["TempStat"].Visible = true;
            ...
            dgvReport.Columns[29].Visible = false;
        ...
        private void dgvReport_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            WorkflowController.ByQty = byQty;
            WorkflowController.TargetOrder = selectedOrder;
        ...
        private void dgvReport_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 0)
            {
                if ((from split in ObjectModels.Tables.ProductionSplits
                     where split.Commessa == _orderToUpdate
                     select split).ToList().Count > 0)
                {

                    WorkflowController.TargetOrder = _orderToUpdate;

                    var sph = new SplitHistory();
                ...
                if (WorkflowController.ListOfRemovedOrders.Contains(_orderToUpdate) && !_lstOfSplittedOrd.Contains(_orderToUpdate))
                {
            ...
            if (e.ColumnIndex == 3)
            {
                ...
                foreach (var str in _listOfAllLines)
                {
                    _cbLineChange.Items.Add(str);
                }
                ...
                e.ColumnIndex == 31 || e.ColumnIndex == 32 || e.ColumnIndex == 33 || e.ColumnIndex == 34)
            {
                ...
                //dgvReport.SelectionChanged += (s, g) =>
                //{
                ...
                //    _txtInput.Text = dgvReport.Rows[e.RowIndex].Cells[e.ColumnIndex].Value.ToString();
                //    dgvReport.Controls.Add(_txtInput);
                //    _rowIdx = dgvReport.SelectedRows[0].Index;
                //    //_cellIdx = e.ColumnIndex;
                //    _txtOnEnter = _txtInput.Text;
                //    _txtInput.Tag = e.ColumnIndex;
                //    Rectangle = dgvReport.GetCellDisplayRectangle(_cellIdx, _rowIdx, true);
                //    _txtInput.Size = new Size(Rectangle.Width, Rectangle.Height);
                //    _txtInput.Location = new Point(Rectangle.X, Rectangle.Y);
                //    _txtInput.Font = new Font("Microsoft Sans Serif", 8);
                //    _txtInput.Visible = true;
        ...
        private void _cbLineChange_SelectedIndexChanged(object sender, EventArgs eventArgs)
        {
            ...
            var d = JobModel.GetLineLastDate(cb.Text);
            ...
            var j = new JobModel();
            var jobDuration = j.CalculateJobDuration(cb.Text, qty, qtyH);    //production duration
            int.TryParse(j.CalculateDailyQty(cb.Text, qtyH).ToString(), out var dailyQty);
            ...
            // update job aim
            {
                ctx.ExecuteCommand("update comenzi set DataProduzione={0},DataFine={1},Line={2}," +
                    "QtyInstead={3},Duration={4} where NrComanda={5}",
                    startDate, endDate,
                    cb.Text, byQty, jobDuration, _orderToUpdate);
            }
            ...
            dgvReport.CurrentRow.Cells[0].Value =
                Properties.Resources.order_statusok_24;
        ...
        private void tmsiSplitCommessa_Click(object sender, EventArgs e)
        {
            ...
            if (UseSingleFilter)
            {
                modelQuery = from model in Central.ListOfModels
                             where model.Name == WorkflowController.TargetOrder
                             && model.Aim == WorkflowController.TargetLine
                             select model;
            }
            else
            {
                ...
                WorkflowController.TargetOrder = _orderToUpdate;
                WorkflowController.TargetLine = _line;
            }
        ...
        private void DgvReport_CellMouseEnter(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                var ln = (from line in ObjectModels.Tables.Lines
                          where line.Line == dgvReport.Rows[e.RowIndex].Cells["Linea"].Value.ToString()
                          select line);
        ...
        private void ResetToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            // update job aim
            {
                ctx.ExecuteCommand("delete from objects where ordername={0} and aim={1}",
                    _orderToUpdate, line);
            }
        ...
        public void LoadExcelData()
        {
            ...
            foreach (DataGridViewRow row in dgvReport.Rows)
            {
                ...
                DateTime.TryParse(row.Cells["Rdd"].Value.ToString(), out var rdd);
                var note = row.Cells["Note"].Value.ToString();
                DateTime.TryParse(row.Cells["Liv"].Value.ToString(), out var conseg);
                DateTime.TryParse(row.Cells["Lans"].Value.ToString(), out var arrivo);
        ...
        {
            var str = "";
            var q = (from art in ObjectModels.Tables.Articles
                     where art.Articol == a
