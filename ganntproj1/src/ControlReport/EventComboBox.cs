...
namespace ganntproj1
    {
    public class EventComboBox : ComboBox
        {
        ...
        private class DropdownWindow : NativeWindow
            {
            private readonly EventComboBox mParent;
            private int mItem;
            public DropdownWindow(EventComboBox parent)
                {
                mParent = parent;
                mItem = -1;
                }
            ...
            private void OnSelect(bool scroll)
                {
                SendMessageRc(this.Handle, 0x198, (IntPtr)mItem, out RECT rc);
